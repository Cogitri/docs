...
>> **Enable Admin State :** Laissez cette option activée pour activer le réseau.<br>
>> **Create Subnet :** Laissez cette option activée pour créer le sous-réseau.<br>
...
>> **Allocation Pools :** Facultatif. Vous pouvez spécifier la plage dans laquelle les IP sont sélectionnées.<br>
...
Dans l'interface de l'horizon, cliquez sur `Compute`{.action} dans le menu de gauche, puis `Instances`{.action}.
...
|---|---|
|Nom de l'instance|Spécifiez le nom souhaité pour l'instance qui sera lancée.|
|Description|Facultatif. Définissez une description, le cas échéant.|
|Zone de disponibilité|Laisser « nova » (choix par défaut).|
|Nombre|Spécifiez le nombre d'instances à lancer.|
